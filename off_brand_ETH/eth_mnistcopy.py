    ...
    for step, batch in enumerate(tqdm(dataloader)):
        ...
        if plot:
            ...
            if (step%100) == 0:
                ...
                weights = network.connections['X', 'Ae'].w.cpu().numpy()
                print("Saved file")
                np.savetxt(f"ETH_Weight_step_{step}_{epoch}.csv", weights,
                    delimiter=',')
                ex_spikes = spike_record[step % update_interval].cpu().numpy()
                np.savetxt(f"ETH_ExcSpikes_step_{step}_{epoch}.csv",
                    xxx, delimiter=',')
                np.savetxt(f"ETH_InputSpikes_step_{step}_{epoch}.csv",
                    yyy, delimiter=',')
